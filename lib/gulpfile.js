...
gulp.task('css', ['cleanCompile'], () => {
  ...
  return merge2([
    gulp.src('assets/' + '*.less').pipe(less()),
...
function babelifyInternal(js, modules) {
  ...
  // transform [name].scss & [name].less to [name].css
  babelConfig.plugins.push([require.resolve('babel-plugin-transform-rename-import'), [
    { original: '^(.+?)\\.(less|scss)$', replacement: '$1.css' },
  ]]);

  return js.pipe(babel(babelConfig))
    .pipe(through2.obj(function (file, encoding, next) {
      const lessPath =
        new RegExp(`(["\']${require(resolveCwd('package.json')).name})\/assets\/([^.\'"]+).less`, 'g');
